...
```sh
go run ./cmd/tpty game create --game-id tutorial --data games/tutorial --seed1 1 --seed2 2
```
...
wrote games/tutorial/game.json
```

You now have a game. It's the manifest under `games/tutorial/` — but it has no
...
```sh
go run ./cmd/tpty world generate --rings 3 --data games/tutorial
```
...
wrote 37 provinces to games/tutorial/world.json
wrote terrain translation to games/tutorial/terrain-translation.json
```
...
Open `games/tutorial/world.json`. The top looks like this:
...
own seeds from the game's. Your game's seeds are in `games/tutorial/game.json`.
...
`--game-id tutorial-2 --data games/tutorial-2 --seed1 1 --seed2 3`, and generate
its world.)

## What you did

You created your first game, generated a world for it, inspected its provinces,
and saw that the seeds make it reproducible. You can delete `games/tutorial/` and
start over any time.
